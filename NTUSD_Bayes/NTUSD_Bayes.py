...
class NTUSD_Bayes(object):
	...
	def __init__(self):
		self.trainfile = 'NTUSD_0&1.xlsx'
		self.testfile = '534test.xlsx'
		self.resultnum = 2
		self.stopwords = []
		self.Dict = {}
		self.P_pos = 0
	...
	def Result(self):
		...
		for i in range(ws1.max_row):
			...
			if ws1['H{0}'.format(i+1)].value != None:
				...
				if original == 0 and predict !=5:
					count_ori_0 += 1
				elif original == 1 and predict !=5:
					count_ori_1 += 1
			else:
				original = 5
		...
		wb.save('result_NB_{0}.xlsx'.format(self.resultnum))
